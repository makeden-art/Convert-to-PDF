...
@app.post("/api/draft/{draft_id}/save-to-smb")
async def api_draft_save_to_smb(draft_id: str, body: DraftSaveRequest):
    ...
    import shutil
    try:
        if _is_smb_path(Path(body.path)) and _smb_mounted():
            from converter import _smb_put_file
            _smb_put_file(final_pdf, Path(body.path))
        else:
            shutil.copy2(str(final_pdf), body.path)
    except Exception as e:
...
@app.get("/api/draft/{draft_id}/preview")
...
async def api_draft_preview(draft_id: str, page: int = 1):
    ...
    try:
        ...
        png, meta = await asyncio.wait_for(
            ...
            asyncio.to_thread(render_preview_png, str(draft_pdf), page=page, variant="source"),
