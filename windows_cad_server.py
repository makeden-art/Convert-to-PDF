...
(defun GetFrames ( / ss i ent edata pts xmin ymin xmax ymax w h frames obj ll ur blkName layName logf)
  ...
  (defun IsValidName (name)
    ...
    (setq name2 (strcase name))
    (or (vl-string-search "ФОРМАТ" name2) 
        (vl-string-search "РАМКА" name2) 
        (vl-string-search "ШТАМП" name2)
        (vl-string-search "ФОРМАТ" name) 
        (vl-string-search "формат" name) 
        (vl-string-search "Формат" name) 
        ...
        (vl-string-search "штамп" name)
        (vl-string-search "Штамп" name)
        (vl-string-search "FORM" name2) 
        (vl-string-search "FRAME" name2) 
        (vl-string-search "STAMP" name2))
  )
    (setq name2 (strcase name))
    (or (vl-string-search "ФОРМАТ" name2) 
        (vl-string-search "РАМКА" name2) 
